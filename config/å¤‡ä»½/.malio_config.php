...
$Malio_Config['login_style'] = 'simple';    // 登录页面的样式，可选 simple 和 wallpaper
$Malio_Config['enable_landing_page'] = true;    // 是否启用着陆页用于介绍本站
$Malio_Config['malio_js_version'] = 'v5';    // 可以随便写，每次更新 malio.js 文件就要改变这个值，用户浏览器就会请求最新的js授权文件，就不会出现缓存的问题
$Malio_Config['small_brand'] = 'LC';    // 侧边栏在缩小状态下显示的 logo 名称，建议写两个英文字母或一个中文汉字
...
$Malio_Config['force_user_to_bind_tg_when_join_group'] = false;   // 设置为true时，限制只有绑定了tg的用户才能加入群组，如未绑定将会被踢出群组。.config.php 需要设置群组id，机器人在群组中不回应设为false才能用。机器人需要在群里组设定为管理员才能踢人。(定制功能)
...
$Malio_Config['mups_alipay'] = 'stripe';   // Malio 聚合支付系统里面的 支付宝 要用的支付平台  
...
$Malio_Config['stripe_currency'] = 'hkd';   // Stripe 支付接口的默认货币，可以写 hkd usd等，stripe限制了收款货币只能是账号注册地区的货币
...
####### 在线客服系统 #######
// Crisp 设置
$Malio_Config['enable_crisp'] = true;   // 是否启用 Crisp 在线客服系统 https://crisp.chat
$Malio_Config['enable_crisp_outside'] = true;   // 是否对未登录的用户也启用 Crisp，设置为 false 的话，着陆页和登录/注册等页面不会显示 Crisp，同时对 Chatra 也是生效的
$Malio_Config['crisp_wesite_id'] = '5fdd5cc6-cc16-436f-99ee-d34f14b4dbcf';    // Crisp 的网站ID，格式为 '18b46e92-eb21-76d3-bfb7-8f2ae9adba64'
...
$Malio_Config['telegram_group'] = '';   // Telegram 按钮的链接
...
$Malio_Config['index_hidden_import_buttons'] = ['copy-ss','copy-ssr','ssd','copy-vmess','ss','ssr'];
...
$Malio_Config['shop_trail_plan_shopid'] = '1';   // 新用户试用的商品ID
