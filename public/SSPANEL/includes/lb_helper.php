...
class LicenseBoxAPI{
	...
	public function verify_license($time_based_check = false, $license = false, $client = false){
		...
		if($time_based_check && $this->verification_period > 0){
			...
			$today = date('d-m-Y');
			if(empty($_SESSION["b0c329e486b4a7f"])){
				$_SESSION["b0c329e486b4a7f"] = '00-00-0000';
			}
			if($type == 1){
				$type_text = '1 day';
			}elseif($type == 3){
			...
			if(strtotime($today) >= strtotime($_SESSION["b0c329e486b4a7f"])){
				$get_data = $this->call_api(
					'POST',
