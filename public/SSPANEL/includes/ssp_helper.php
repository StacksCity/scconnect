...
define("LB_TEXT_UPDATE_WITH_SQL_DONE", '更新成功，请在数据库中导入下载的sql文件。');
define("LB_TEXT_UPDATE_WITHOUT_SQL_DONE", '更新成功，没有SQL更新。因此，您可以直接运行更新的应用程序。');
...
class LicenseBoxAPI{
	...
	public function __construct(){ 
		$this->product_id = '6472AC82';
		$this->api_url = 'https://www.benew.top/';
		$this->api_key = '2AAA11550ABAAF64D9F4';
		$this->api_language = 'chinese';
		$this->current_version = 'v2.1';
	...
	public function verify_license($time_based_check = false, $license = false, $client = false){
		...
		if($time_based_check && $this->verification_period > 0){
			...
			$today = date('d-m-Y');
			if(empty($_SESSION["f143101ad10c313"])){
				$_SESSION["f143101ad10c313"] = '00-00-0000';
			}
			if($type == 1){
				$type_text = '1 day';
			}elseif($type == 3){
			...
			if(strtotime($today) >= strtotime($_SESSION["f143101ad10c313"])){
				$get_data = $this->call_api(
					'POST',
